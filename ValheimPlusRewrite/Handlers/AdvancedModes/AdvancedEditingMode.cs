...
{
    [ConfigHandler(typeof(AdvancedEditingModeConfiguration))]
    internal class AdvancedEditingMode : AdvancedModeBase
    {
        ...
        [HarmonyPostfix]
        private static void Player_Update_Postfix(ref Player __instance, ref Vector3 ___m_moveDir, ref Vector3 ___m_lookDir, ref GameObject ___m_placementGhost, Transform ___m_eye)
        {
        ...
        public static bool ExecuteRayCast(Player playerInstance)
        {
            ...
            if (Physics.Raycast(
                ...
                Vector3.Distance(playerInstance.ToCharacter().m_eye.position, raycastHit.point) < playerInstance.m_maxPlaceDistance)
            {
        ...
        public static void run()
        {
            ...
            // CHECK FOR ABM
            if (AdvancedBuildingMode.isActive)
            {
                if (isActive)
                {
                    exitMode();
            ...
            if (isActive)
            {
                ...
                if (hitPieceStillExists())
                {
                    ...
                    try
                    {
                        ...
                        {
                            Log.LogInfo("AEM: Error, network object empty. Code: 2.");
                            exitMode();
                            return;
                        }
                    }
                    ...
                    {
                        Log.LogInfo("AEM: Error, network object empty. Code: 3.");
                        exitMode();
                    }
        ...
        private static void listenToHotKeysAndDoWork()
        {
            ...
            if (Input.GetKeyDown(Configuration.Current.AdvancedEditingMode.ConfirmPlacementOfAdvancedEditingMode))
            {
                ...
                if ((UnityEngine.Object)component1 == (UnityEngine.Object)null)
                {
                    Log.LogInfo("AEM: Error, network object empty.");
        ...
        {
            MessageHud.instance.ShowMessage(MessageHud.MessageType.TopLeft, "AEM: " + Message);
        }

        private static void notifyUser(string Message, MessageHud.MessageType position)
        {
