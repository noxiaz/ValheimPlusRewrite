...
{
    [ConfigHandler(typeof(AdvancedBuildingModeConfiguration))]
    internal class AdvancedBuildingMode : AdvancedModeBase
    {
        // Status
        ...
        [HarmonyPrefix]
        private static bool Player_UpdatePlacementGhost_Prefix(ref Player __instance, bool flashGuardStone)
        {
            if (isActive)
            {
                return false;
        ...
        [HarmonyPatch(typeof(Player), nameof(Player.UpdatePlacementGhost))]
        [HarmonyPostfix]
        private static void Player_UpdatePlacementGhost_Postfix(Player __instance, bool flashGuardStone)
        {
            if (isActive)
            {
                return;
        ...
        [HarmonyPostfix]
        private static void Player_Update_Patch(ref Player __instance, ref Vector3 ___m_moveDir, ref Vector3 ___m_lookDir, ref GameObject ___m_placementGhost, Transform ___m_eye)
        {
        ...
        public static void Run(ref Player __instance)
        {
            ...
            if (AdvancedEditingMode.isActive)
            {
                if (isActive)
                {
                    exitMode();
        ...
        {
            MessageHud.instance.ShowMessage(MessageHud.MessageType.TopLeft, "ABM: " + Message);
        }
